...
jQuery( window.document ).ready(function($){
     ...
     * Show Welcome
     */
    function pstoolkit_show_welcome() {
        if (
            'undefined' === typeof SUI ||
            ! $( '#pstoolkit-welcome' ).length
        ) {
            window.setTimeout( pstoolkit_show_welcome, 100 );
        } else {
            SUI.openModal( 'pstoolkit-welcome', this, null, true );
            $('button.pstoolkit-welcome-all-modules').on( 'click', function () {
                var dialog = $(this).closest( '.sui-modal');
                var data = {
                    action: 'pstoolkit_welcome_get_modules',
                ...
                jQuery.post(ajaxurl, data, function( response ) {
                    if ( response.success ) {
                        dialog
                            ...
                            .removeClass( 'pstoolkit-welcome-step1' ).addClass( 'pstoolkit-welcome-step2' )
                        ;
                        ...
                        ;
                        window.pstoolkit_modules_mark_all( '.pstoolkit-group-checkbox', dialog );
                        window.pstoolkit_modules_save_bulk( '.pstoolkit-welcome-activate', dialog, true );
    ...
    pstoolkit_show_welcome();
    /**
     * Search modules on dashboard
     */
    $('#pstoolkit-dashboard-widget-summary-search')
        .on( 'change keydown keyup blur reset copy paste cut input', function() {
            var search = $(this).val();
            var target = $('#pstoolkit-dashboard-widget-modules');
            var re;
            if ( '' === search ) {
                $('tr, .sui-box', target ).show();
                $('#pstoolkit-dashboard-search-no-results').hide();
                return;
            }
            ...
            if ( 1 > $('table:visible', target).length ) {
                $('#pstoolkit-dashboard-search-no-results').show();
                $('#pstoolkit-dashboard-search-no-results span').html( search );
            } else {
                $('#pstoolkit-dashboard-search-no-results').hide();
            }
        });
    if ( $( '#pstoolkit-notice-permissions-settings-data' ).length ) {
        var $data = $( '#pstoolkit-notice-permissions-settings-data' );
        SUI.openFloatNotice( $data.html(), 'success', true, undefined, 'pstoolkit-notice-permissions-settings' );
        $( '#pstoolkit-notice-permissions-settings' ).data( 'nonce', $data.data( 'nonce' ) ).data( 'id', $data.data( 'id' ) );
    /**
     * Message
     */
    $( '#pstoolkit-notice-permissions-settings .sui-icon-check' ).on( 'click', function() {
        var $notice = $( '#pstoolkit-notice-permissions-settings' );
        var data = {
            action: 'pstoolkit_notice_permissions_notice_save',
...
jQuery( window.document ).ready(function( $ ){
    ...
    $( '.simple-option-file .sui-upload-file button' ).on( 'click', function() {
        ...
        SUI.closeNotice( 'pstoolkit-wrong-filetype' );
    });
    /**
     * bind change
     */
    jQuery( '.module-utilities-import-php .pstoolkit-upload').on( 'change', function(e) {
        ...
        if ( '' === value ) {
            ...
            if ( re.test( value ) ) {
                ...
                SUI.openInlineNotice( 'pstoolkit-wrong-filetype', ub_admin.messages.export.not_json );
            }
        }
    });
});
...
function ub_media_bind( obj, event ) {
    ...
    SUI.closeNotice('pstoolkit-only-images');
    ...
    file_frame.on( 'select', function() {
		...
		if (attachment.hasOwnProperty('mime') && !attachment.mime.includes('image/')) {
			SUI.openInlineNotice( 'pstoolkit-only-images', ub_admin.messages.common.only_image );
			return;
		} else {
			SUI.closeNotice('pstoolkit-only-images');
		}
        ...
        if ( 'content_background' === container.data( 'id' ) ) {
            jQuery('canvas.pstoolkit-focal', container.closest('form') )
...
jQuery( window.document ).ready( function( $ ) {
    ...
    $('.pstoolkit-add-image').on('click', function() {
        var ub_template = wp.template( 'simple-options-media' );
...
jQuery( window.document ).ready(function($){
    ...
    function pstoolkitFormatSelection( site ) {
	...
	var args = {
            ...
            templateResult: pstoolkitFormat,
            templateSelection: pstoolkitFormatSelection,
...
SUI.pstoolkitSaveSettings = function($, form) {
...
jQuery( window.document ).ready(function($){
     ...
     */
    $.fn.extend( {
        pstoolkit_editor: function( editor_id ) {
        ...
        pstoolkit_flag_status: function( element ) {
            var $header = $('[data-tab=' + $( element ).closest('.sui-box.pstoolkit-settings-tab').data( 'tab' ) + '] .sui-box-status');
            /**
             ...
             */
            if ( 0 < $( element ).closest( '.pstoolkit-avoid-flag' ).length ) {
                return;
            }
            window.pstoolkit_has_changed = 'unsaved';
            $( '.pstoolkit-status-changes-saved', $header ).hide();
            $( '.pstoolkit-status-changes-unsaved', $header ).show();
        },
        pstoolkit_generate_id: function() {
            var text = "";
     ...
     * reset section
     */
    $('.pstoolkit-reset-section').on('click', function() {
        var dialog_id = 'pstoolkit-dialog-reset-section';
        var $dialog = $('#'+dialog_id);
        var $button = $('.pstoolkit-data-reset-confirm', $dialog );
    ...
    $('#pstoolkit-dialog-reset-section .pstoolkit-data-reset-confirm').on( 'click', function() {
        var data = {
     ...
     */
    function pstoolkit_ace_editor_placeholder( id, placeholder ) {
    ...
    jQuery( '.sui-ace-editor' ).each( function() {
        ...
        if ( '' !== $(this).prop( 'placeholder' ) ) {
            SUI.editors[id].on( 'input', function() {
                pstoolkit_ace_editor_placeholder( id, $(this).prop( 'placeholder' ) );
            });
            pstoolkit_ace_editor_placeholder( id, $(this).prop( 'placeholder' ) );
        }
     ...
     * SUI: save
     */
	var $module_save_button = $('.pstoolkit-module-save');
	$module_save_button.on('click', function () {
		window.pstoolkit_has_changed = 'saving';
	});

$module_save_button.on( 'click', function () { SUI.pstoolkitSaveSettings.call(this, jQuery) });
    /**
     * reset whole module
     */
    $(' .pstoolkit-reset-module' ).on( 'click', function() {
        var dialog_id = 'pstoolkit-dialog-reset-module-' + $(this).data('module');
        if ( 'undefined' === $( '#' + dialog_id ) ) {
            if ( window.confirm( ub_admin.messages.reset.module ) ) {
                var args = {
                    action: 'pstoolkit_reset_module',
                    module: $(this).data('module'),
    ...
    $( '.pstoolkit-dialog-reset-module button.pstoolkit-reset' ).on( 'click', function() {
        var args = {
            action: 'pstoolkit_reset_module',
            module: $(this).data('module'),
            _wpnonce: $(this).data('nonce')
     ...
     */
    $.each( $('.pstoolkit-social-logo-preview'), function() {
        var target = $(this);
     ...
     * social media
     */
    function pstoolkit_social_media_bind_delete() {
        $('button.pstoolkit-social-logo-remove' ).on( 'click', function() {
            var parent = $(this).closest('.sui-accordion-item-body');
            $( '.pstoolkit-social-logo-add-dialog-button', parent ).removeAttr( 'disabled' );
            var target = $('.pstoolkit-social-logo-add-dialog .pstoolkit-social-logo-li-'+$(this).data('id'), parent );
            target.removeClass( 'hidden' );
            $( 'input', target ).removeAttr( 'checked' );
            $(this).closest('.sui-form-field').detach();
        });
    }
    pstoolkit_social_media_bind_delete();

    $('.pstoolkit-social-logo-add-accounts' ).on( 'click', function() {
        var parent = $(this).closest('.sui-box');
        var items = $('li:not(.hidden) :checked', parent );
        var pstoolkit_template = wp.template( $(this).data( 'template' ) );
        items.each( function() {
            $('.pstoolkit-social-logos-main-container', $(this).closest( 'form' ) ).append(
                pstoolkit_template({
                    id: $(this).val(),
        ...
        if ( 0 === $( 'li', parent ).not('.hidden').length ) {
            $('.pstoolkit-social-logo-add-dialog-button', $(this).closest('.sui-accordion-item-body') ).prop( 'disabled', true );
        }
        SUI.closeModal();
        /**
     ...
     */
    function pstoolkit_move_focal( context, el, x, y ) {
        var canvas = el.get(0);
        ...
        context.fillStyle = $('.sui-wrap-pstoolkit').hasClass( 'sui-color-accessible' )? '#000':'#17A8E3';
        context.fill();
        context.lineWidth = 3;
        context.strokeStyle = '#fff';
        context.stroke();
        $( 'input.pstoolkit-focal-x', parent ).val( nx );
        $( 'input.pstoolkit-focal-y', parent ).val( ny );
        $( 'span.pstoolkit-focal-x', parent ).html( nx );
        $( 'span.pstoolkit-focal-y', parent ).html( ny );
        el.css( 'backgroundPosition', nx + '% ' + ny + '%' );
        return context;
    }
    $('canvas.pstoolkit-focal').each( function() {
        var el = $(this);
        var parent = el.closest( '.sui-form-field' );
        var canvas = el.get(0);
        var context = canvas.getContext('2d');
        var x = Math.round( canvas.width * $( 'input.pstoolkit-focal-x', parent ).val() / 100 );
        var y = Math.round( canvas.height * $( 'input.pstoolkit-focal-y', parent ).val() / 100 );
        context = pstoolkit_move_focal( context, el, x, y );
        canvas.addEventListener('mousedown', function(e) {
            context = pstoolkit_move_focal( context, el, e.offsetX, e.offsetY );
        }, true);
     ...
     */
    $( '.ub-radio.pstoolkit-social-media-show' ).on( 'click', function() {
        var elements = $('.simple-option-sui-tab', $(this).closest( '.sui-box-body' ) );
        if ( $(this).is(':checked' ) && 'on' === $(this).val() ) {
            elements.removeClass( 'pstoolkit-not-affected' );
            ...
            $.each( elements, function() {
                if ( ! first ) {
                    $(this).addClass( 'pstoolkit-not-affected' );
                }
                first = false;
            });
        }
     ...
     * Indicator
     */
    window.pstoolkit_has_changed = 'saved';
    window.onbeforeunload = function() {
        if ( 'unsaved' === window.pstoolkit_has_changed ) {
            return ub_admin.messages.unsaved;
        }
    };
    $( '.pstoolkit-settings-tab-content input, .pstoolkit-settings-tab-content textarea, .pstoolkit-settings-tab-content select' ).on( 'change', function() {
	var not_settings = $( this ).closest( '.pstoolkit-box-actions' ).length || $( this ).closest( '.sui-table' ).length;
	if ( ! not_settings ) {
		$.fn.pstoolkit_flag_status( this );
	}
    });
...
jQuery( window.document ).ready(function(){
    ...
    if ( jQuery.fn.switchButton ) {
        var pstoolkit_admin_check_slaves  = function() {
...
jQuery( window.document ).ready(function($){
    ...
    $(document).on( 'click', '.sui-wrap-pstoolkit .sui-sidenav .sui-vertical-tab a', function(event) {
        ...
        if ( tab === window.pstoolkit_current_tab ) {
            return;
        }
...
jQuery( window.document ).ready( function($) {
	...
	if ( $( '.pstoolkit-new-features' ).length ) {
		var modal_id = $( '.pstoolkit-new-features' ).prop( 'id' );
	...
	// Hit on Got it.
	$( '.pstoolkit-new-feature-got-it' ).on( 'click', function() {
		var data = {
			action: 'pstoolkit_new_feature_dismiss',
    ...
    $( '.ub-activate-module, .ub-deactivate-module' ).on( 'click', function() {
        var data = {
            action: 'pstoolkit_toggle_module',
	 ...
	 * Manage all modules
	 */
	function pstoolkit_modules_mark_all( selector, parent ) {
    ...
    pstoolkit_modules_mark_all( '#pstoolkit-manage-all-modules .pstoolkit-group-checkbox, .pstoolkit-import-import .pstoolkit-group-checkbox', 'body' );
	 ...
	 */
	function pstoolkit_modules_save_bulk( selector, parent, is_welcome ) {
		jQuery( selector, parent ).on( 'click', function() {
			...
			jQuery('input[type=checkbox]:checked', dialog ).each( function() {
				...
				if ( value ) {
					pstoolkit.push( value );
				}
			});
			if ( 0 === pstoolkit.length ) {
				if ( is_welcome ) {
					SUI.openFloatNotice( window.ub_admin.messages.welcome.empty, 'warning' );
					return false;
				} else {
					pstoolkit = 'turn-off-all-modules';
				}
			}
			...
			button.prop('disabled', true );
			var data = {
				action: 'pstoolkit_manage_all_modules',
				pstoolkit: pstoolkit,
    ...
    pstoolkit_modules_save_bulk( '#pstoolkit-manage-all-modules .sui-box-footer button.pstoolkit-save-all', 'body', false );

	/**
	 * Reset dialog content
	 */
    $('.pstoolkit-dialog-reset').on( 'click', function() {
	 ...
	 * @since 1.0.0
	 */
    function pstoolkit_copy_settings_check( dialog ) {
        var module_id = $('.pstoolkit-copy-settings-select option:selected', dialog ).val();
        var sections = $('.pstoolkit-copy-settings-' + module_id + ' .pstoolkit-copy-settings-section:checked');
        if ( 1 > sections.length ) {
            $('.pstoolkit-copy-settings-copy-button', dialog ).prop( 'disabled', true );
        } else {
            $('.pstoolkit-copy-settings-copy-button', dialog ).removeAttr( 'disabled' );
        }
    }
    $('.pstoolkit-copy-settings-select').on( 'change', function() {
        var dialog = $(this).closest('.sui-modal');
        var module_id = $(':selected', $(this) ).val();
        $('.pstoolkit-copy-settings-options', dialog ).hide();
        if ( '' !== module_id ) {
            $('.pstoolkit-copy-settings-copy-button', dialog ).prop( 'disabled', true );
            $('.pstoolkit-copy-settings-' + module_id ).show();
        }
        pstoolkit_copy_settings_check( dialog );
    });
    $('.pstoolkit-copy-settings-section').on( 'change', function() {
        var dialog = $(this).closest('.sui-modal');
        pstoolkit_copy_settings_check( dialog );
    });
    $('.pstoolkit-copy-settings-copy-button').on( 'click', function() {
        var dialog = $(this).closest('.sui-modal');
        var module_id = $('.pstoolkit-copy-settings-select option:selected', dialog ).val();
        var data = {
            action: 'pstoolkit_module_copy_settings',
	 ...
	 * @since 1.0.0
	 */
    $( 'input[type=checkbox].pstoolkit-cb-select-all' ).on( 'change', function() {
        var parent = $(this).closest( '.pstoolkit-settings-tab' );
        if ( $(this).is(':checked') ) {
            $('.check-column input[type=checkbox]:not(.pstoolkit-cb-select-all)', parent ).prop( 'checked', true ).filter( ':first' ).trigger( 'change' );
        } else {
            $('.check-column input[type=checkbox]:not(.pstoolkit-cb-select-all)', parent ).removeProp( 'checked' ).filter( ':first' ).trigger( 'change' );
        }
    });
    $( '.pstoolkit-bulk-delete' ).on( 'click', function() {
        var parent = $(this).closest( '.pstoolkit-settings-tab' ),
	 ...
	 * @since 1.0.0
	 */
    $( '.pstoolkit-test-email .sui-box-footer button' ).on( 'click', function() {
        var $button = $(this);
...
(function($) {
	...
	SUI.openFloatNotice = function ( text, type, can_dismiss, close_after, id ) {
		SUI.openNoticePSToolkit( true, id, text, type, can_dismiss, close_after );
	}
	...
	SUI.openInlineNotice = function ( notice_id, text, type, can_dismiss, close_after ) {
		SUI.openNoticePSToolkit( false, notice_id, text, type, can_dismiss, close_after );
	}
	 ...
	 * @param {int} close_after
	 */
	SUI.openNoticePSToolkit = function ( float, notice_id, text, type, can_dismiss, close_after ) {
		if ( type === undefined ) {
			type = 'error';
		}
		var icon = 'info';
	...
	SUI.closeNoticePSToolkit = function () {
		try {
			SUI.closeModal();
		} catch (e) {
		}
