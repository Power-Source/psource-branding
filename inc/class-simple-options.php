	...
	class Simple_Options {
		...
		public function build_options( $options, $input, $module ) {
			...
			foreach ( $options as $section_key => $option ) {
				...
				if (
					...
					if ( $this->is_network ) {
						if ( ! is_network_admin() ) {
							$subsite = apply_filters( 'pstoolkit_module_check_for_subsite', false, $module, null );
							if ( $subsite ) {
								continue;
							}
						}
				...
				if ( preg_match( '/^(default|accordion)$/', $show_as ) ) {
					...
					if ( ! $section_inside ) {
						if ( isset( $option['no-sui-columns'] ) && true === $option['no-sui-columns'] ) {
							$classes[] = 'pstoolkit-no-sui-columns';
						}
				...
				foreach ( $option['fields'] as $id => $data ) {
					...
					if ( 'accordion' === $show_as && isset( $data['accordion'] ) ) {
						if ( isset( $data['accordion']['begin'] ) && $data['accordion']['begin'] ) {
							$accordion_item_classes = array(
								'sui-accordion-item',
								sprintf( 'pstoolkit-accordion-item-%s-%s', $module, $section_key ),
							);
							if (
					...
					if (
						...
						if ( isset( $data['group']['label'] ) ) {
							...
							if ( isset( $data['group']['after_label'] ) ) {
								...
								$group_label_classes[] = 'pstoolkit-has-actions';
							}
							/**
							 * units on group
							 */
					...
					switch ( $data['type'] ) {
						...
						case 'wp_editor':
							...
							if ( isset( $data['placeholder'] ) ) {
								$content .= sprintf(
									'<div class="pstoolkit-editor-placeholder hidden" aria-hidden="true">%s</div>',
						...
						case 'sui-tab-icon':
							...
							foreach ( $data['options'] as $radio_value => $radio_data ) {
								...
								if (
									...
									if (
										...
										&& in_array( 'pstoolkit-icon', $radio_data['classes'] )
									) {
										$file  = pstoolkit_dir( 'assets/images/icons/' );
							...
							if (
								...
								foreach ( $content_tabs[ $section_key ][ $data['slave-class'] ] as $slave_id => $slave ) {
									...
									$content .= sprintf(
										'<div class="%s%s pstoolkit-tab-%s-%s" data-tab-content="%s">',
						...
						default:
							switch ( $data['type'] ) {
								...
								case 'time':
									$data['classes'][]    = 'pstoolkit-input-time';
							...
							if ( 'link' === $data['type'] ) {
								...
								if ( 'password' === $data['type'] ) {
									...
									$content .= sprintf(
										...
										esc_html__( 'Passwort verbergen', 'ub' )
									);
									$content .= '</button>';
									$content .= '</div>';
								} else {
					...
					if ( false && isset( $data['default'] ) ) {
						...
						if ( $show ) {
							...
							if ( ! empty( $default ) || is_numeric( $default ) ) {
								$message = sprintf(
									__( 'Standard ist: <code><strong>%s</strong></code>', 'ub' ),
									$default
								);
								if ( 'color' == $data['type'] ) {
									$message = sprintf(
										__( 'Die Standardfarbe ist: <code><strong>%s</strong></code>', 'ub' ),
										$data['default']
									);
								}
				...
				if ( preg_match( '/^(default|accordion)$/', $show_as ) ) {
					 ...
					 * add reset
					 */
					$show_reset_button = apply_filters( 'pstoolkit_options_show_reset', false );
					 ...
					 */
					$table_footer = apply_filters( 'pstoolkit_options_footer', $table_footer, $module, $section_key );
					/**
			...
			if ( empty( $content ) ) {
				$content = sprintf(
					...
					__( 'Hier sind keine Optionen definiert.', 'ub' )
				);
			} else {
				/**
				...
				if ( isset( $options['reset-module'] ) && true === $options['reset-module'] ) {
					$uba                      = pstoolkit_get_uba_object();
					$show_reset_module_button = apply_filters( 'pstoolkit_options_show_reset_module_button', true, $module );
				 ...
				 * Filter content
				 */
				$content .= apply_filters( 'pstoolkit_options_after', '', $module );
			}
			return $content;
		}
			...
			if ( isset( $_REQUEST['do'] ) && isset( $_REQUEST['nonce'] ) ) {
				switch ( $_REQUEST['do'] ) {
					case 'copy':
						...
						break;
					default:
						wp_send_json_error( array( 'message' => __( 'Falsche Handlung!', 'ub' ) ) );
				}
			}
			wp_send_json_error( array( 'message' => __( 'Etwas ist schief gelaufen!', 'ub' ) ) );
		}
		...
		public function ajax_reset_section() {
			...
			if (
				...
				wp_send_json_error( array( 'message' => __( 'Fehlende erforderliche Parameter.', 'ub' ) ) );
			}
			...
			if ( 'unknown' == $option_name ) {
				$success = apply_filters( 'pstoolkit_reset_section', $success, $module, $section );
			} else {
				$reset_section = apply_filters( 'pstoolkit_reset_section_' . $module, false, $option_name, $section );
				...
				if ( $reset_section ) { // Specific resetting section
					...
					wp_send_json_error( array( 'message' => __( 'Abschnitt ist bereits im Standardzustand!', 'ub' ) ) );
				}
			}
			if ( $success ) {
				$uba     = pstoolkit_get_uba_object();
				$message = array(
					'type'    => 'info',
					'message' => __( 'Abschnittsdaten wurden zurückgesetzt.', 'ub' ),
				);
		...
		private function copy_settings( $to, $section, $from ) {
			$from_data = pstoolkit_get_option( $from );
			if ( ! isset( $from_data[ $section ] ) ) {
				wp_send_json_error( array( 'message' => __( 'Der Quellabschnitt ist noch nicht gespeichert und es gibt nichts zu kopieren!', 'ub' ) ) );
			}
			$to_data = pstoolkit_get_option( $to );
			if ( ! is_array( $to_data ) ) {
				$to_data = array();
			}
			...
			$result = pstoolkit_update_option( $to, $to_data );
			if ( $result ) {
				$uba     = pstoolkit_get_uba_object();
				$message = array(
					'type'    => 'info',
					'message' => __( 'Abschnittsdaten wurden erfolgreich kopiert.', 'ub' ),
				);
				$uba->add_message( $message );
			}
			return $result;
		...
		private function enqueue_jquery_style() {
			...
			wp_enqueue_style( $key, pstoolkit_url( 'assets/css/vendor/jquery-ui.min.css' ), array(), '1.12.1' );
		...
		private function image( $section_key, $id, $images, $type, $module ) {
			...
			$element_id = sprintf(
				'pstoolkit_option_media_%s_%s_%s',
				PSToolkit_Helper::hyphen_to_underscore( sanitize_title( $module ) ),
		...
		public function add_media_template() {
			$args     = array(
				'button' => __( 'Bild hochladen', 'ub' ),
				'label'  => __( 'Datei löschen', 'ub' ),
			);
		...
		private function add_units( $data, $input, $section_key, $id ) {
			...
			if ( 1 === sizeof( $units ) ) {
				...
				$content .= sprintf(
					'<select class="sui-select-sm pstoolkit-units" name="%s">',
					esc_attr( $field_name )
				);
		...
		public function add_teeny_mce_placeholder_plugin( $mceInit, $editor_id ) {
			$file = pstoolkit_url( 'external/wp-tinymce-placeholder/mce.placeholder.js' );
			if ( preg_match( '/^pstoolkit/', $editor_id ) ) {
