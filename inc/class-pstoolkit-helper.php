...
if ( ! class_exists( 'PSToolkit_Helper' ) ) {
	...
	class PSToolkit_Helper {
		 ...
		 * @since 1.9.4
		 */
		protected $module = 'PSToolkit_Helper';
		...
		public function __construct() {
			...
			if ( is_admin() ) {
				...
				if ( is_a( $uba, 'PSToolkit_Admin' ) ) {
					$this->uba = $uba;
				} else {
					$this->uba = new PSToolkit_Admin();
				}
			...
			add_filter( 'pstoolkit_options_names', array( $this, 'add_option_name' ) );
			add_filter( 'pstoolkit_get_option_name', array( $this, 'get_module_option_name' ), 10, 2 );
			/**
			 * Add data copy
			 */
			add_filter( 'pstoolkit_options_footer', array( $this, 'add_copy_options' ), 10, 3 );
			/**
			 ...
			 * @since 3.1.0
			 */
			add_filter( 'pstoolkit_options_show_reset_module_button', array( $this, 'show_reset_module_button' ), 10, 2 );
		}
		...
		protected function get_value( $section = null, $name = null, $default = null ) {
			$this->set_data();
			   $data = $value = is_array($this->data) ? $this->data : array();
			if ( ! empty( $section ) ) {
				   if ( empty( $name ) && isset( $value[ $section ] ) ) {
					   $data = is_array($value[ $section ]) ? $value[ $section ] : array();
				} else {
					/**
		...
		public function admin_options_page( $content ) {
			if ( method_exists( $this, 'set_options' ) ) {
				...
				do_action( 'pstoolkit_admin_options_page_before_options', $this->option_name );
				return $simple_options->build_options( $this->options, $this->data, $this->module );
			}
			return self::sui_notice( esc_html__( 'Etwas ist schief gelaufen!', 'ub' ) );
		}

		protected function set_data() {
			if ( null === $this->data ) {
				$value = pstoolkit_get_option( $this->option_name );
				if ( 'empty' !== $value ) {
					$this->data = $value;
				}
			}
		...
		public function update( $status ) {
			...
			if ( empty( $this->options ) ) {
				if ( self::is_debug() ) {
					$msg = sprintf( 'PSToolkit Admin: Leeres Optionsarray für %s Variable. Bitte wende Dich an Plugin-Entwickler.', $this->option_name );
					error_log( $msg, 0 );
				}
				return;
			}
			...
			foreach ( $this->options as $section_key => $section_data ) {
				...
				foreach ( $section_data['fields'] as $key => $data ) {
					...
					switch ( $data['type'] ) {
						 ...
						 */
						case 'wp_editor':
							$v                                       = do_shortcode( stripslashes( $value[ $section_key ][ $key ] ) );
							$v                                       = wpautop( $v );
						...
						case 'text':
							if ( isset( $value[ $section_key ][ $key ] ) ) {
								$value[ $section_key ][ $key ] = stripslashes( $value[ $section_key ][ $key ] );
							}
							break;
						default:
							break;
			 ...
			 */
			$filter   = sprintf(
				'pstoolkit_settings_%s_preserve',
			 ...
			 * do action
			 */
			do_action( 'pstoolkit_helper_update', $this->module, $value, $current_value );
			/**
			 * save & return
			 */
		...
		protected function update_value( $value ) {
			...
			pstoolkit_update_option( $this->option_name, $value );
			$this->data = $value;
			return true;
		}
		...
		public function change_bottom_save_button( $args, $module ) {
			if ( $this->module === $module['module'] ) {
				...
				$args['text'] = __( 'Einstellungen speichern', 'ub' );
			}
			return $args;
		}
		...
		public function disable_save() {
			add_filter( 'pstoolkit_settings_panel_show_submit', '__return_false' );
		}
		...
		protected function load_social_logos_css( $module_key = '' ) {
			...
			if ( ! empty( $module_key ) ) {
				...
				if ( ! empty( $module ) ) {
					...
					$check = pstoolkit_files_dir( $file );
					if ( is_file( $check ) ) {
						$file = pstoolkit_files_url( $file );
		...
		protected function get_social_logos_css_url() {
			$url = pstoolkit_url( 'external/icon-font/social-logos.css' );
			return $url;
		}
		...
		protected function css_border_options() {
			$options = array(
				'dotted' => __( 'Gepunktet', 'ub' ),
				'dashed' => __( 'Gestrichelt', 'ub' ),
				'solid'  => __( 'Fest', 'ub' ),
				'double' => __( 'Doppelt', 'ub' ),
				'groove' => __( '3D gerillt', 'ub' ),
				'ridge'  => __( '3D geriffelt', 'ub' ),
				'inset'  => __( '3D-Einschub', 'ub' ),
				'outset' => __( '3D-Anfang', 'ub' ),
			);
			return $options;
		}
		...
		protected function css_background_common( $selector = 'html', $echo = true, $wrap = true ) {
			...
			$args      = array(
				'id'                    => $this->get_name(),
				'selector'              => $selector,
				'selector2'             => sprintf( '%s, .pstoolkit-background-mask', $selector ),
		...
		protected function get_template( $file = 'index' ) {
			...
			return __( 'Etwas ist schief gelaufen!', 'ub' );
		}
		...
		protected function get_options_background( $defaults = array() ) {
			$data = array(
				'title'  => __( 'Hintergrund', 'ub' ),
				'fields' => array(
					'color'    => array(
						'type'    => 'color',
						'label'   => __( 'Hintergrundfarbe', 'ub' ),
						'default' => isset( $defaults['color'] ) ? $defaults['color'] : '#210101',
					),
					'show'     => array(
						'type'        => 'checkbox',
						'label'       => __( 'Hintergrundbilder anzeigen', 'ub' ),
						'description' => __( 'Möchtest Hintergrundbilder verwenden?', 'ub' ),
						'options'     => array(
					...
					'mode'     => array(
						'type'    => 'radio',
						'label'   => __( 'Modus für mehrere Bilder', 'ub' ),
						'options' => array(
							'slideshow' => __( 'Diashow', 'ub' ),
							'random'    => __( 'Zufällig', 'ub' ),
						),
						'default' => 'slideshow',
					...
					'image'    => array(
						'type'        => 'gallery',
						'label'       => __( 'Hintergrundbild', 'ub' ),
						'description' => __( 'Hier kannst Du ein Hintergrundbild hochladen. Das Bild wird an die Seite angepasst und automatisch geändert, wenn sich die Fenstergröße ändert. Du erzielst die besten Ergebnisse, wenn Du Bilder mit einer Mindestbreite von 1024 Pixel verwendest.', 'ub' ),
						'master'      => 'ub-default-background-show-related',
					),
					'duration' => array(
						'type'        => 'number',
						'label'       => __( 'Dauer der Diashow', 'ub' ),
						'description' => __( 'Dauer in Minuten, wir empfehlen dringend, nicht weniger als 5 Minuten zu verwenden.', 'ub' ),
						'default'     => 10,
						'min'         => 1,
						'max'         => 60,
						'after'       => __( 'Minuten', 'ub' ),
		...
		protected function get_options_fields( $group, $sections, $defaults = array() ) {
			...
			foreach ( $sections as $section ) {
				...
				if ( is_callable( $function ) ) {
					...
					error_log( sprintf( 'Fehlender Rückruf: %s', $function[1] ) );
				}
			}
			return $data;
		}
			...
			$data = array(
				'reset' => array(
					...
					'container-classes' => array(
						'pstoolkit-section-reset',
					),
					'classes'           => array(
						'pstoolkit-reset-section',
					),
				),
			);
			return $data;
		...
		protected function get_options_fields_content_logo( $defaults = array() ) {
			$data = array(
				'logo_image'         => array(
					...
					'label'        => __( 'Logo', 'ub' ),
					'description'  => array(
						'content'  => __( 'Die bevorzugte Breite des Logos beträgt 320 Pixel, um die besten Ergebnisse zu erzielen.', 'ub' ),
						'position' => 'bottom',
					),
				...
				'logo_url'           => array(
					...
					'label'        => __( 'URL', 'ub' ),
					'description'  => array(
						'content'  => __( 'Benutzer werden zu diesem Link weitergeleitet, wenn sie auf das Logo klicken.', 'ub' ),
						'position' => 'bottom',
					),
				...
				'logo_show'          => array(
					'type'        => 'sui-tab',
					'label'       => __( 'Sichtbarkeit des Logos', 'ub' ),
					'options'     => array(
						'off' => __( 'Ausblenden', 'ub' ),
						'on'  => __( 'Anzeigen', 'ub' ),
					),
				...
				'content_background' => array(
					...
					'label'       => __( 'Hintergrundbild', 'ub' ),
					'description' => array(
						'content'  => __( 'Für beste Ergebnisse sollte das Bild eine Mindestbreite von 1024 Pixel haben. Du kannst mehrere Bilder hochladen und im Designbereich eine Diashow erstellen.', 'ub' ),
						'position' => 'bottom',
					),
					'accordion'   => array(
						'end' => true,
		...
		protected function get_options_fields_content_error_message( $defaults = array() ) {
			$data = array(
				...
				'content_content' => array(
					'type'      => 'wp_editor',
					'label'     => __( 'Beschreibung (optional)', 'ub' ),
		...
		protected function get_options_fields_content_social( $defaults = array() ) {
			 ...
			 */
			$args     = array(
				'buttom_open_label' => __( 'Öffne Element', 'ub' ),
			...
			$data['add-new-button'] = array(
				...
				'classes'   => array(
					'pstoolkit-social-logo-add-dialog-button',
				),
				'data'      => array(
			...
			$data[ $key ]['accordion']['title'] = __( 'Soziale Konten', 'ub' );
			if ( 1 > count( $data ) ) {
				$key = 'add-new-button';
			}
			$data['add-new-button']['before_field']  = '';
			$data[ $key ]['before_field']            = '<div class="sui-accordion pstoolkit-sui-accordion-sortable social-logo-color pstoolkit-social-logos-main-container">';
			...
			$content  = sprintf(
				...
				esc_html__( 'Wähle die Plattformen aus, die in Dein Social-Sharing-Modul eingefügt werden sollen.', 'ub' )
			);
			...
			foreach ( $social_media as $k => $value ) {
				...
				$content .= sprintf(
					'<li class="pstoolkit-social-logo-li-%s%s" data-id="%s"><label for="%s" class="sui-box-selector">',
					esc_attr( $k ),
				...
				$content .= '<span class="pstoolkit-social-logo-container">';
				$content .= sprintf(
			...
			$args                             = array(
				'text'  => __( 'Konten hinzufügen', 'ub' ),
				'sui'   => '',
				'class' => 'pstoolkit-social-logo-add-accounts ' . $this->get_name( 'add' ),
				'data'  => array(
			...
			$args                             = array(
				...
				'title'        => __( 'Soziales Konto hinzufügen', 'ub' ),
				'confirm_type' => false,
				'classes'      => array( 'pstoolkit-social-logo-add-dialog', 'social-logo-color', 'sui-modal-lg' ),
			);
		...
		protected function get_options_fields_design_logo( $defaults = array() ) {
			$data = array(
				'logo_width'         => array(
					...
					'description'  => array(
						'content'  => __( 'Es sollte kleiner sein als das Formular Canvas (320px).', 'ub' ),
						'position' => 'bottom',
					),
				),
				'logo_opacity'       => array(
					'type'         => 'number',
					'label'        => __( 'Deckkraft', 'ub' ),
					'min'          => 0,
					'max'          => 100,
		...
		protected function get_options_fields_design_background( $defaults = array() ) {
			$data = array(
				'background_duration'      => array(
					'type'         => 'number',
					'label'        => __( 'Dauer', 'ub' ),
					'after_label'  => __( 'Sekunden', 'ub' ),
					'description'  => array(
						'content'  => __( 'Wir empfehlen dringend eine Dauer von mindestens 5 Sekunden.', 'ub' ),
						'position' => 'bottom',
					),
					'default'      => 10,
					'min'          => 1,
				...
				'background_crop'          => array(
					...
					'options'     => array(
						'auto'   => array(
							'label'   => __( 'Auto', 'ub' ),
							'tooltip' => __( 'Schnitt nicht ändern.', 'ub' ),
						),
						'height' => array(
							'label'   => __( 'Höhe', 'ub' ),
							'tooltip' => __( 'Schnitt proportional, Höhe ändern.', 'ub' ),
						),
						'width'  => array(
							'label'   => __( 'Breite', 'ub' ),
							'tooltip' => __( 'Schnitt proportional, Breite ändern.', 'ub' ),
						),
						'fixed'  => array(
							'label'   => __( 'Fest', 'ub' ),
							'tooltip' => __( 'Auf feste Größe zuschneiden', 'ub' ),
						),
					),
				...
				'background_attachment'    => array(
					'type'        => 'sui-tab',
					'label'       => __( 'Hintergrundbefestigung', 'ub' ),
					'description' => array(
						'content' => __( 'Diese CSS-Eigenschaft legt fest, ob eine Hintergrundbildposition im Ansichtsfenster festgelegt ist oder mit dem enthaltenen Block gescrollt wird.', 'ub' ),
					),
					'options'     => array(
						'unset'  => array(
							'label'   => __( 'Nicht eingestellt', 'ub' ),
							'tooltip' => __( 'Ändert sich nicht.', 'ub' ),
						),
						'fixed'  => array(
							'label'   => __( 'Fixiert', 'ub' ),
							'tooltip' => __( 'Der Hintergrund ist relativ zum Ansichtsfenster festgelegt. Selbst wenn ein Element über einen Bildlaufmechanismus verfügt, bewegt sich der Hintergrund nicht mit dem Element.', 'ub' ),
						),
						'scroll' => array(
							'label'   => __( 'Scrollen', 'ub' ),
							'tooltip' => __( 'Der Hintergrund ist relativ zum Element selbst festgelegt und scrollt nicht mit seinem Inhalt.', 'ub' ),
						),
					),
					'default'     => 'unset',
				),
				'background_size_width'    => array(
					'type'         => 'number',
					'description'  => array(
						'content'  => __( 'Belasse "0" für "auto"-Wert.', 'ub' ),
						'position' => 'bottom',
					),
					'label'        => __( 'Breite', 'ub' ),
					'after_label'  => __( '%', 'ub' ),
				...
				'background_size'          => array(
					'type'        => 'sui-tab',
					'label'       => __( 'Hintergrundgröße', 'ub' ),
					'options'     => array(
						'cover'   => array(
							'label'   => __( 'Cover', 'ub' ),
							'tooltip' => __( 'Ändere die Größe des Hintergrundbilds, um den gesamten Container zu bedecken, auch wenn das Bild gedehnt oder an einer der Kanten etwas abgeschnitten werden muss.', 'ub' ),
						),
						'contain' => array(
							'label'   => __( 'Enthalten', 'ub' ),
							'tooltip' => __( 'Ändere die Größe des Hintergrundbilds, um sicherzustellen, dass das Bild vollständig sichtbar ist.', 'ub' ),
						),
						'none'    => array(
							'label'   => __( 'Automatisch', 'ub' ),
							'tooltip' => __( 'Das Hintergrundbild wird in seiner Originalgröße angezeigt.', 'ub' ),
						),
						'manual'  => array(
							'label'   => __( 'Länge', 'ub' ),
							'tooltip' => __( 'Legt die Breite und Höhe des Hintergrundbilds fest.', 'ub' ),
						),
					),
				...
				'background_position_x'    => array(
					'type'    => 'sui-tab-icon',
					'label'   => __( 'Horizontale Hintergrundposition', 'ub' ),
					'options' => array(
						'left'   => array(
							'label' => 'align-x-left',
						),
				...
				'background_position_y'    => array(
					'type'      => 'sui-tab-icon',
					'label'     => __( 'Vertikale Hintergrundposition', 'ub' ),
					'options'   => array(
						'top'    => array(
							'label' => 'align-y-top',
						),
		...
		protected function get_options_fields_design_document( $defaults = array() ) {
			$data = array(
				'content_width'  => array(
					...
					'label'        => __( 'Breite', 'ub' ),
					'description'  => array(
						'content'  => __( 'Für beste Ergebnisse sollte es vorzugsweise weniger als 1024 sein.', 'ub' ),
						'position' => 'bottom',
					),
		...
		protected function get_options_fields_design_social( $defaults = array() ) {
			$data = array(
				'social_media_show'   => array(
					...
					'accordion'   => array(
						'begin' => true,
						'title' => __( 'Soziale Konten', 'ub' ),
					),
					'classes'     => array(
						'pstoolkit-social-media-show',
					),
					'description' => array(
						'content'  => self::sui_notice( __( 'Die folgenden Optionen sind nicht betroffen.', 'ub' ), 'warning' ),
						'position' => 'bottom',
					),
				),
				'social_media_target' => array(
					'type'    => 'sui-tab',
					'label'   => __( 'Öffne Link', 'ub' ),
					'options' => array(
						'_blank' => __( 'Neuer Tab', 'ub' ),
						'_self'  => __( 'Im gleichen', 'ub' ),
					),
					'default' => esc_attr( isset( $defaults['target'] ) ? $defaults['target'] : '_self' ),
				),
				'social_media_colors' => array(
					'type'              => 'sui-tab',
					'label'             => __( 'Icon-Stil', 'ub' ),
					'options'           => array(
						'color'      => __( 'Farben', 'ub' ),
						'monochrome' => __( 'Einfarbig', 'ub' ),
					),
		...
		protected function get_options_social_media_settings( $defaults = array() ) {
			$data = array(
				'title'  => __( 'Social Media-Einstellungen', 'ub' ),
				'fields' => array(
					'show'                         => array(
						'type'        => 'checkbox',
						'label'       => __( 'Zeige', 'ub' ),
						'description' => __( 'Möchtest Du Social Media zeigen?', 'ub' ),
						'options'     => array(
							'off' => __( 'Ausblenden', 'ub' ),
							'on'  => __( 'Anzeigen', 'ub' ),
						),
					...
					'colors'                       => array(
						'type'        => 'checkbox',
						'label'       => __( 'Farben', 'ub' ),
						'description' => __( 'Möchtest Du farbige Symbole anzeigen?', 'ub' ),
						'options'     => array(
							'on'  => __( 'Farben', 'ub' ),
							'off' => __( 'Einfarbig', 'ub' ),
						),
					...
					'social_media_link_in_new_tab' => array(
						'type'        => 'checkbox',
						'label'       => __( 'Öffne Links', 'ub' ),
						'description' => __( 'Möchtest Du einen Link in einem neuen oder demselben Fenster/Tab öffnen?', 'ub' ),
						'options'     => array(
							'on'  => __( 'Neu', 'ub' ),
							'off' => __( 'Selben', 'ub' ),
						),
		...
		public function add_copy_options( $content, $module, $section ) {
			...
			if ( 0 ) {
				...
				$content    .= '<select>';
				$content    .= sprintf( '<option value="-1">%s</option>', esc_html__( '-- Kopie von --', 'ub' ) );
				...
				$content .= '</select>';
				$content .= sprintf( '<a href="#" class="button">%s</a>', esc_html__( 'Kopieren', 'ub' ) );
				$content .= '</label></div>';
			}
			return $content;
		}
		...
		private function add_related( $related, $section ) {
			...
			$uba           = pstoolkit_get_uba_object();
			$dir           = pstoolkit_files_dir( 'modules/' );
		...
		protected function common_options_document( $defaults = array(), $remove = array() ) {
			$args = array(
				'title'  => __( 'Dokument', 'ub' ),
				'fields' => array(
					'title_show'              => array(
						'type'        => 'checkbox',
						'label'       => __( 'Titel anzeigen', 'ub' ),
						'description' => __( 'Möchtest Du den Titel zeigen?', 'ub' ),
						'options'     => array(
							'off' => __( 'Aus', 'ub' ),
							'on'  => __( 'An', 'ub' ),
						),
					...
					'content_show'            => array(
						'type'        => 'checkbox',
						'label'       => __( 'Inhalt zeigen', 'ub' ),
						'description' => __( 'Möchtest Du Inhalte anzeigen?', 'ub' ),
						'options'     => array(
							'off' => __( 'Aus', 'ub' ),
							'on'  => __( 'An', 'ub' ),
						),
					...
					'background_transparency' => array(
						'type'    => 'number',
						'label'   => __( 'Hintergrundtransparenz', 'ub' ),
						'min'     => 0,
						'max'     => 100,
						'default' => 0,
						'classes' => array( 'ui-slider' ),
		...
		public function rename_option_name() {
			if ( ! empty( $this->old_option_name ) ) {
				$value = pstoolkit_get_option( $this->old_option_name );
				if ( ! empty( $value ) ) {
					$this->update_value( $value );
					pstoolkit_delete_option( $this->old_option_name );
				}
			}
		}
		...
		public function upgrade_plugin_settings() {
			 ...
			 */
			$is_active = pstoolkit_is_active_module( 'custom-login-css.php' );
			if ( $is_active ) {
				if ( ! is_a( $this->uba, 'PSToolkit_Admin' ) ) {
					$this->uba = new PSToolkit_Admin();
				}
		...
		public function get_options_link_in_new_tab( $defaults = array() ) {
			$args = array(
				...
				'label'   => __( 'Öffne Link', 'ub' ),
				'options' => array(
					'off' => __( 'Gleiche Registerkarte', 'ub' ),
					'on'  => __( 'Neue Registerkarte', 'ub' ),
				),
				'default' => 'off',
			);
		...
		public function reset_module( $status ) {
			pstoolkit_delete_option( $this->option_name );
			return true;
		}
		...
		protected function sui_dialog( $args ) {
			...
			$content .= '<button class="sui-button-icon sui-button-float--right" data-modal-close>'
					...
					. esc_html__( 'Schließe dieses Modal', 'ub' )
					. '</span></button>';
			/**
			 * Title
			 */
		...
		protected function proceed_sui_config( $config, $widget_id = 0, $tab = '' ) {
			...
			foreach ( $config as $name => $data ) {
				...
				if (
					...
					$content .= '<div class="pstoolkit-divider"></div>';
				}
				$classes = array(
					'sui-form-field',
					sprintf(
				...
				switch ( $type ) {
					...
					case 'number':
						...
						$content .= sprintf(
							'<input id="%s" type="%s" name="pstoolkit[%s]" value="%s" data-default="%s" data-required="%s" %s />',
							esc_attr( $id ),
							esc_attr( $type ),
							esc_attr( $name ),
							esc_attr( $value ),
					...
					case 'checkboxes':
						...
						foreach ( $data['options'] as $checkbox_value => $checkbox_label ) {
							...
							$content .= sprintf(
								'<input type="checkbox" name="pstoolkit[%s][%s]" value="%s" %s %s />',
								esc_attr( $name ),
					...
					case 'wp_editor':
						$args = array(
							'textarea_name' => sprintf( 'pstoolkit[%s]', $name ),
							'textarea_rows' => 9,
							'teeny'         => true,
						);
						ob_start();
					...
					case 'html_editor':
						...
						$content          .= sprintf(
							'<textarea id="%s" name="pstoolkit[%s]" class="%s">%s</textarea>',
							esc_attr( $id ),
							esc_attr( $name ),
					...
					case 'sui-tab':
						...
						foreach ( $data['options'] as $radio_value => $radio_label ) {
							...
							$content .= sprintf(
								'<input type="radio" name="pstoolkit[%s]" value="%s" %s data-name="%s" data-tab-menu="%s" data-default="%s" />',
								esc_attr( $name ),
							...
							&& isset( $data['content'][ $radio_value ] )
								...
								$tabs_content .= sprintf(
									'<div class="sui-tab-boxed pstoolkit-tab-%s%s" data-tab-content="%s">',
									esc_attr( $tab_id ),
									$value === $radio_value ? ' active' : '',
									esc_attr( $tab_id )
								);
					...
					case 'callback':
						if ( isset( $data['callback'] ) && is_callable( $data['callback'] ) ) {
							...
							$content .= __( 'Etwas ist schief gelaufen!', 'ub' );
						}
						break;
					default:
						break;
				...
				if ( isset( $data['required'] ) && $data['required'] ) {
					$content .= sprintf(
						...
						__( 'Dieses Feld kann nicht leer sein!', 'ub' )
					);
				}
				/**
				 * description
		...
		public function get_name( $sufix = null, $module = null ) {
			...
			$name   = sprintf( 'pstoolkit-%s', $module );
		...
		public function get_dialog_delete( $id, $args = array() ) {
			$defaults = array(
				'description' => __( 'Möchtest Du das ausgewählte Element wirklich löschen?', 'ub' ),
				'title'       => __( 'Bist du sicher?', 'ub' ),
			);
			 ...
			 * @since 1.0.0
			 */
			$attr    = apply_filters( 'pstoolkit_dialog_delete_attr', $attr, $this->module, $id );
			$content = sprintf(
		...
		private function enqueue_module_admin_assets( $module = null ) {
			...
			$file     = pstoolkit_files_dir( $filename );
			// Check if file exists.
			if ( is_file( $file ) ) {
				$file = pstoolkit_files_url( $filename );
				// Register script.
				...
				$localize = apply_filters( 'pstoolkit_localize_script', array(), $module );
				if ( ! empty( $localize ) ) {
					$name = $this->get_name();
			 ...
			 * @since 3.1.0
			 */
			do_action( 'pstoolkit_admin_enqueue_module_admin_assets', $module );
		}
		...
		public function ajax_search_subsite() {
			...
			if ( 'images' === $this->module ) {
				$exclude[] = pstoolkit_get_main_site_id();
			}
			if (
		...
		public static function is_pro() {
			$is_pro = defined( 'PSTOOLKIT_BUILD_TYPE' ) && 'full' === PSTOOLKIT_BUILD_TYPE;
		...
		protected function get_custom_css_array( $config = '' ) {
			$description = __( 'Verwende für erweiterte Anpassungsoptionen benutzerdefiniertes CSS.', 'ub' );
			if (
		...
		public function render( $file, $params = array(), $return = false ) {
			...
			$file          = trim( $file, '/' );
			$template_file = pstoolkit_dir( 'views/' . $file ) . '.php';
			if ( file_exists( $template_file ) ) {
				ob_start();
				include $template_file;
				$content = ob_get_clean();
			} elseif ( self::is_debug() ) {
				error_log( __( 'Vorlagendatei existiert nicht!', 'ub' ) );
				error_log( $template_file );
				if ( current_user_can( 'manage_options' ) ) {
					$message  = sprintf(
						__( 'Vorlagendatei %s existiert nicht!', 'ub' ),
		...
		protected function generate_id( $args ) {
			 ...
			 * get algorithm
			 */
			$algoritm = apply_filters( 'pstoolkit_get_id_algoritm', 'crc32' );
			$allowed  = hash_algos();
		...
		protected function get_users_can_register_notice() {
			...
			$notice = array(
				...
				'value' => self::sui_notice(
					sprintf(
						__( 'Die Benutzerregistrierung wurde deaktiviert. Klicke <a href="%s">hier</a>, um die Benutzerregistrierung für Deine Website zu aktivieren.', 'ub' ),
						$url
					)
				),
			);
		...
		protected function set_roles( $sort = true, $add_super = true ) {
			...
			if ( $add_super && $this->is_network ) {
				$roles = array_merge( array( 'super' => __( 'Netzwerkadministrator', 'ub' ) ), $roles );
			}
			if ( $sort ) {
				asort( $roles );
			}
		...
		protected function maybe_crop( $image, $id ) {
			...
			switch ( $crop ) {
				...
				break;
				case 'fixed':
					$width  = $this->get_value( 'design', 'background_crop_width' );
					$height = $this->get_value( 'design', 'background_crop_height' );
					if (
					$image['meta'][1] === $width
					&& $image['meta'][2] === $height
					) {
