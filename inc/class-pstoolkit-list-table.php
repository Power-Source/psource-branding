<?php
if ( ! class_exists( 'PSToolkit_List_Table' ) ) {
	if ( ! class_exists( 'WP_List_Table' ) ) {
		require_once ABSPATH . 'wp-admin/includes/class-wp-list-table.php';
	}
	class PSToolkit_List_Table extends WP_List_Table {
		protected $url;
		protected $config;
		...
		protected function pstoolkit_bulk_actions( $which, $dialog_id ) {
			...
			echo '<label for="bulk-action-selector-' . esc_attr( $which ) . '" class="screen-reader-text">' . __( 'Massenaktion auswählen', 'ub' ) . '</label>';
			echo '<select name="action" class="sui-select-sm sui-select-inline" id="bulk-action-selector-' . esc_attr( $which ) . "\">\n";
			echo '<option value="-1">' . __( 'Massenaktion', 'ub' ) . "</option>\n";
			$actions = array(
				'delete' => __( 'Delete', 'ub' ),
			);
		...
		/**
		 * Get PSToolkit Heaser column CB
		 *
		 * @since 1.0.0
		 */
		protected function get_header_column_cb_all() {
			...
			$content  = sprintf(
				...
				esc_attr( $id ),
				esc_html__( 'Alles auswählen', 'ub' )
			);
			$content .= sprintf(
				'<label for="%s" class="sui-checkbox"><input id="%s" type="checkbox" class="pstoolkit-cb-select-all" /><span aria-hidden="true"></span></label>',
				esc_attr( $id ),
				esc_attr( $id )
			);
			return $content;
		...
		protected function pagination( $which ) {
			...
			$output               = '<span class="sui-pagination-results">' . sprintf( _n( '%s Element', '%s Elemente', $total_items, 'ub' ), number_format_i18n( $total_items ) ) . '</span>';
